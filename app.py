...
columns = ['Option Type', 'Instrument Key', 'LTP', 'Bid Price', 'Bid Qty', 'Ask Price', 'Ask Qty', 'Volume', 'OI', 'Prev OI', 'OI Change',
           ...
           'Call Sum | Calculated', 'Put Sum | Calculated', 'Options Flow Diff | Calculated']
table_data = []
...
row_call = ['Call', instrument_key_call, market_data_call.get('ltp', 0), market_data_call.get('bidPrice', 0), market_data_call.get('bidQty', 0),
            ...
            'Bullish' if oi_change_call > 0 else 'Bearish' if oi_change_call < 0 else 'Neutral', 'Day Trend' if volume_call > 0 and oi_call <= volume_call else 'Long-Term Trend' if volume_call > 0 and oi_call > volume_call else 'No Trend',
...
row_put = ['Put', instrument_key_put, market_data_put.get('ltp', 0), market_data_put.get('bidPrice', 0), market_data_put.get('bidQty', 0),
           ...
           'Bullish' if oi_change_put > 0 else 'Bearish' if oi_change_put < 0 else 'Neutral', 'Day Trend' if volume_put > 0 and oi_put <= volume_put else 'Long-Term Trend' if volume_put > 0 and oi_put > volume_put else 'No Trend',
